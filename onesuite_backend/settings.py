...
# SECRET_KEY is REQUIRED - will raise error if not set in environment
...
WSGI_APPLICATION = 'onesuite_backend.wsgi.application'
...
if config('DATABASE_URL', default=None):
    # If DATABASE_URL is provided, we might want to parse it. 
    # But sticking to the requested individual vars for now or default sqlite for local dev if no output.
    pass
...
if DB_ENGINE == 'django.db.backends.postgresql':
    DATABASES = {
        'default': {
            ...
            'OPTIONS': {
                'sslmode': 'require',
            } if not config('DB_HOST', '').startswith('/') else {},
        }
    }
else:
    DATABASES = {
...
AUTHENTICATION_BACKENDS = [
    'users.backends.EmailOrUsernameModelBackend',  # Allow login with email or username
